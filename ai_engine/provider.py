...
class AIEngine:
    ...
    def _call_gemini_sdk(self, prompt: str, system_instruction: str) -> AIResponse:
        try:
            ...
            if "401" in err_msg or "403" in err_msg: 
                raise AuthenticationError("API Key Gemini không hợp lệ hoặc bị từ chối truy cập.")
            if "404" in err_msg: 
                raise ModelNotFoundError("Model không tồn tại hoặc không được hỗ trợ bởi API Key này.")
            if "429" in err_msg or "quota" in err_msg: 
                raise QuotaExceededError("Quá giới hạn request Gemini (429).")
            if "timeout" in err_msg: 
                raise TimeoutError("Hết thời gian chờ kết nối đến Gemini.")
    ...
    def _call_openai(self, prompt: str, system_instruction: str) -> AIResponse:
        try:
            ...
            if "timeout" in err_msg: 
                raise TimeoutError("Timeout kết nối máy chủ OpenAI.")
            raise NetworkError(f"Lỗi OpenAI: {str(e)}")
    ...
    def _call_anthropic(self, prompt: str, system_instruction: str) -> AIResponse:
        try:
            ...
            if "timeout" in err_msg: 
                raise TimeoutError("Timeout kết nối máy chủ Claude.")
            raise NetworkError(f"Lỗi Claude: {str(e)}")
    ...
    def _call_openrouter(self, prompt: str, system_instruction: str) -> AIResponse:
        try:
            ...
            if "404" in err_msg:
                raise ModelNotFoundError("Model không tồn tại trên OpenRouter.")

            if "402" in err_msg or "429" in err_msg or "credits" in err_msg.lower():
                raise QuotaExceededError("Hết Credits hoặc máy chủ OpenRouter quá tải.")

            if "timeout" in err_msg.lower():
                raise TimeoutError("Hết thời gian chờ (Timeout) khi gọi OpenRouter.")
    ...
    def _call_ollama(self, prompt: str, system_instruction: str) -> AIResponse:
        ...
        try:
            ...
            raise NetworkError("Ollama chưa chạy trên máy local hoặc không thể truy cập từ môi trường hiện tại.")
        except requests.exceptions.Timeout:
            raise TimeoutError("Timeout khi kết nối đến Ollama.")
        except Exception as e:
            raise NetworkError(f"Lỗi kết nối Ollama: {str(e)}")
